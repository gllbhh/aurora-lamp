...
//WiFi credentials
const char* ssid = "MobileFabLab";  //replace "WiFi_name" with your WIFi name (const char* ssid = "myWiFi";)
const char* password = "oulu2022";  //replace "WiFi_password" with your WIFi name (password = "myPassword";)
...
const int latitude = 65;  //latitude rounded to the nearest integer. Can be from -90 to 90
const int longitude = 25; //longitude rounded to the nearest integer. Can NOT be negative! 0 - 359
const int nthresh = 50; //notification threshold in %. 50 means change animation if probability of Aurora is >= 50%
...
void runNeopixelCode( void * pvParameters ) {
  ...
  for (;;) {
    ...
    //run animation 1
    if (auroraState < nthresh){
        noAuroraAnimation();
        Serial.println("No aurora animation");
    }
    //else run aurora animation
    else {
         auroraAnimation();
         Serial.println("Aurora probability over threshold");
    }
...
void loop() {
   ...
   * But you can't delete it. Without it, Arduino will fail to build the sketch.
   */
}


//getAurora gets Aurora probability in %
...
int getAurora(int longitude, int latitude) {
  ...
  if (http.GET() == HTTP_CODE_OK) {
    ...
    int aurora = 0;
    while (aur != ']'){
      aurora = 10 * aurora + aur - '0';
      aur = pStream->read();
    }
    if(aurora > 100) aurora = 0; //Filter false reading
    return aurora;
  }
    else {
...
void noAuroraAnimation(){
  //Animation that runs when there isn't auroras
  //Replace rainbowCycle(20); with your own animation if you want.
  rainbowCycle(20);  
}

void auroraAnimation(){
  //Animation that runs when there are auroras
  //Replace breathe(10); with your own animation if you want.
  breathe(10);  
}
